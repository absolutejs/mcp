...
test("native registrations share reviewed policy without an account or client allowlist", () => {
  for (const uri of ["http://127.0.0.1:5137/callback", "http://localhost:5000/auth", "http://[::1]:5500/callback", "vscode://extension/callback", "https://vscode.dev/redirect", "https://insiders.vscode.dev/redirect"]) {
...
test("host restrictions, unknown callbacks, mixed channels and review expiry remain enforced", () => {
  for (const uris of [[], ["https://localhost.attacker.test/callback"], ["https://claude.ai.attacker.test/auth"], ["https://user:password@localhost/auth"], ["javascript:alert(1)"], ["https://insiders.vscode.dev.attacker.test/redirect"], ["https://insiders.vscode.dev/not-redirect"]]) expect(registeredMcpProfiles(uris)).toEqual(["unknown"]);
